...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sqlPersona = "UPDATE persona SET nombre=?, apellido=?, direccion=?, telefono=?, email=? WHERE CI=?";
        ...
        $sqlIdPersona = "SELECT id_persona FROM persona WHERE CI=?";
        ...
        if ($rowIdPersona = $resultIdPersona->fetch_assoc()) {
            $id_persona = $rowIdPersona['id_persona'];

            // Actualizar datos en la tabla "empleados"
            $sqlEmpleado = "UPDATE empleado SET cargo_empleado=?, sueldo_empleado=? WHERE id_persona=?";
            $stmtEmpleado = $conn->prepare($sqlEmpleado);
            $stmtEmpleado->bind_param("ssi", $cargo_empleado, $sueldo_empleado, $id_persona);

            if ($stmtEmpleado->execute()) {
                echo '<script>alert("Empleado actualizado exitosamente!");';
                echo 'window.location.href = "listaempleados.php";</script>';
            } else {
                echo '<script>alert("Error al actualizar empleado: ' . $stmtEmpleado->error . '");</script>';
            }
        }
